...
- grammatical_gender: "feminine", "masculine" or "neutral" — how to conjugate adjectives and greetings ABOUT THEM in gendered languages (es/pt). Two sources only, and "gender_stated" says which:
  (a) STATED — they tell you outright how to address them: "call me she", "I'm a woman", "use masculine with me", "my pronouns are they/them" (→ "neutral"), "trátame en femenino", "save my pronouns as she". A BARE pronoun set on its own line is also a statement and must be captured: "she/her" → feminine, "he/him" → masculine, "they/them" → neutral (Lana offers exactly those three as tappable options, so they arrive as the whole message — a user who taps one and is not saved has been asked for nothing). Set gender_stated TRUE. A stated value OVERRIDES whatever is on file, including an earlier stated one — someone correcting us is always right.
  (b) INFERRED — their own gendered SELF-reference in a gendered language ("estoy cansada" → feminine, "estou animado" → masculine) or an explicit self-label ("I'm his mom" → feminine, "I'm their dad" → masculine). Set gender_stated FALSE.
  null otherwise, with gender_stated false. NEVER guess it from their NAME (Andrea, Kim, Alex, Sasha, Noor and Jordan all flip by language — a wrong guess misgenders a real person, an absent one costs nothing), never from a third party, and never from what they search for. Used only for grammatical agreement: never shown back to them, never a claim.
- NEVER extract race, exact age, sex/gender demographics, street address
