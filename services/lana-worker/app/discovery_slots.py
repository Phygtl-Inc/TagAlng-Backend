...
_SYSTEM = (
    ...
    "host a coffee morning, do you know a good pediatrician, anyone want to swap); "
    ...
    "RECOMMEND a local doctor, pediatrician, or clinic ('do you know a good pediatrician', 'recommend a doctor near "
    ...
    "A direct request for a RECOMMENDATION ('do you know a good pizza place', 'recommend a plumber') is "
    ...
    "tip_seek/tip_share = local SERVICE, SHOP or place (teacher, tutor, pediatrician, doctor, "
    "restaurant, plumber, stationery shop, barber); NOT swap_seek, NOT discovery. "
    "signal_category = a 1-2 WORD label for the thing ('dentist', 'stationery', 'pizza'). "
    "education|health|food|home|activities|shopping|services|pets are typical, but use the honest "
    "word when none of those fits — it is a display label, not a filter, so a precise one costs "
    "nothing and a wrong bucket reads back to the user in 'a tip in {category}'. "
    "BUYING LOCALLY IS A TIP; CHOOSING A PRODUCT IS NOT. 'where can I buy art supplies', 'a shop "
    "that sells yarn', 'somewhere to get a haircut' → tip_seek: a nearby place sells it, and a "
    "neighbor can vouch for the place. 'which laptop should I buy', 'is the X400 better than the "
    "Y200', 'best guitar under $80' → out_of_scope: that is product research with no neighborhood "
    "angle, and no neighbor recommendation answers it. 'anyone have a spare drill I could borrow' "
    "→ swap_seek. Decide by what the ANSWER would be: a place → tip, a neighbor's spare thing → "
    "swap, a product verdict → out_of_scope. "
    ...
    "WATCH THE SUBJECT OF 'know': 'do YOU know a good plumber' / 'anyone know a plumber' asks ME "
    "for one and is tip_seek. 'I know a good plumber' / 'we know a great dentist' says THEY have "
    "one and is tip_share — the same words with the opposite meaning. Never read a first-person "
    "'I know a…' as a request. "
    "tip_share (sharing.tip) = the user VOUCHES for one out of their OWN experience, whether or "
    "not they name it. NAMED: 'I recommend Dr Smith', 'try Dr Lee', 'my favorite pizza is Tony's', "
    "'Dr Patel is a great dentist', 'amazing biryani at Zaiqa'. NOT YET NAMED: 'I know a really "
    "reliable plumber', 'I found a great hiking spot', 'we use a great cleaner every two weeks', "
    "'my kids loved the new trampoline park'. "
    "A MISSING NAME IS NOT AN ASK: 'I know a good X' OFFERS one, 'do you know a good X?' REQUESTS "
    "one — the first is always tip_share. Lana's own first question collects the name, so never "
    "downgrade a share to a seek for lacking one. "
    "WORKED EXAMPLES: 'I know a really reliable plumber' → goal=save_signal, "
    "signal_intent=tip_share, linear_intent=sharing.tip (they HAVE one; Lana asks WHICH plumber "
    "next). 'do you know a reliable plumber?' → signal_intent=tip_seek, linear_intent=looking.tip "
    "(they WANT one). 'my kids loved the new trampoline park' → tip_share / sharing.tip, NEVER "
    "identity.add_claim. Same nouns, opposite direction — read who HAS and who WANTS. "
    "Praise for a PLACE or PROVIDER is tip_share, NOT identity.add_claim: 'I found a great hiking "
    "spot' recommends a spot, 'I go hiking every weekend' describes the user. When a message is "
    "both, the recommendation wins — the claim is extracted from the turn either way. "
    ...
    "BUT A COMMUNITY IS NOT A MEET: 'create a COMMUNITY', 'start a community', 'add my gym "
    "as a community', 'make our bakery a community', 'set up a community for the school' is "
    "sharing.community (goal=create_community), NEVER host_meet — even though the verb is "
    "create/start/set up. A meet is ONE gathering at a time ('a coffee morning on Saturday'); "
    "a community is a STANDING PLACE people join and keep coming back to, with no date at "
    "all. The word community, or a named place being turned INTO one, decides it. "
    ...
    "A VERDICT ON A PLACE OR PROVIDER is not self-description: 'I like badminton' is about THEM, "
    "'my kids loved the new trampoline park' / 'we found a great pediatrician' rates a specific "
    "THING they went to → tip_share (sharing.tip). Taste is identity; a review is a recommendation. "
    ...
    "anywhere ('recommend a babysitting service', 'do you know a good plumber?', 'need a tutor/daycare/"
    ...
    "discovery.communities|sharing.community; "
    ...
    "STARTING one is the OPPOSITE DIRECTION and is sharing.community (goal=create_community), "
    "never discovery.communities: 'create a community', 'start a community', 'add my gym as "
    "a community', 'I want to make our bakery a community', 'can I set up a community for "
