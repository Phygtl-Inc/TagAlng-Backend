...
_SYSTEM = (
    ...
    "In preview, rejection of the shown results PLUS new matching criteria "
    "('not these, find me dads', 'show different Brazilian moms', 'instead find soccer parents') "
    "→ linear_intent=discovery.find_by_attrs, goal=peers, in_discovery=true, "
    "attr_filter=new requested traits. This is a fresh search, not chat and not a selection from old cards. "
    "Mid-funnel pushback without new search criteria, or a real topic change, "
    "→ in_discovery=false, goal=chat. "
    ...
    "GENERIC VALUE RECOMMENDATION = the user asks broadly what is useful/worth doing/joining/finding "
    "nearby, without naming one specific place/service/person/item/activity ('got any recommendations "
    "for me?', 'what should I do nearby?', 'anything worth joining?', 'recommend me something'). "
    "For this, set intent_lane=recommendation, intent_action=recommend_value, intent_scope=mixed, "
    "linear_intent=null, goal=chat or none, in_discovery=false, confidence high when clear. "
    "Do NOT use this lane for specific local recommendations like 'recommend a pediatrician/plumber/"
    "coffee place' — those are local_signal + tip + seek (looking.tip / tip_seek). "
