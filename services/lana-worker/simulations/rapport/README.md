...
|---|---|---|
| **facet precision / recall** | mechanical | returned claims and expectations are paired by a **one-to-one maximum bipartite matching** (Kuhn's augmenting paths), and *both* ratios read off that single assignment. Precision = returned claims that got paired; recall = expectations that got paired. Match = `bucket` + `label_has`/`label_has_any` substring + `confidence_min` (+ optional `disclosure`/`transient`/`vague`/`subject_kind`). `subject_kind` (migration `20261021120000`) asserts WHO the claim is about — opt-in, so the 26 pre-subject fixtures are unaffected; it matters because the onion matcher pairs concepts only *within* one subject, so a child's claim mis-stored as `self` would match adults and render the false "you both do karate". **`concept` is NOT matched exactly** — the extractor's slug is a free choice (`karate_practice` vs `karate`), only its regex shape is fixed. |
| **anonymization** | mechanical | `must_not_contain` strings never appear — checked **after** `clean_claims_for_persist` runs, across each claim's `label`/`source_quote`/each `synonym`/`details`/**`concept`** (the slug is built from the user's own words, so real PII rides out in it — backend confirmed `lincoln_elementary_kindergarten`, `sunshine_preschool`; the normalised arm flattens `_` to a space so a needle like "Lincoln Elementary" still matches), the extracted `nickname`, **and** the raw `followup_question` (redaction is never applied to the followup in the real pipeline, so it's a distinct, unprotected leak vector). Fields are tested **separately** and **normalised** (case + punctuation flattened), so punctuation tricks can't hide a leak and a needle can't "match" by straddling two fields. 100% pass required; HARD_FAIL on any leak. |
...
- `subject_kind`, when given, must be one of the `claim_subject_kind` enum labels — a typo would
  silently match nothing and read as a recall miss with no explanation.
