...
| File | What it is |
|---|---|
| `ports.py` | **The swap seam.** 4 Protocols (`MatcherPort`, `DisclosurePort`, `AreaStatePort`, `CircleActivationPort`) + every data model/config that crosses them, now shaped to the as-built schema. Read this first — every field is commented as confirmed or `# RESOLVED (Asjid 2026-07-28[ rev-2])`. `ScoringConfig` reframed as offline re-weighting; `RankedCandidate` carries the RPC's component columns (`same_place_bonus`/`same_type_bonus`/`shared_concept_count`/`shared_concept_labels`/`shared_concept_subjects`/`shared_place_ref`). Claims are **subject-aware** (`SubjectKind`, migrations `20261021`/`20261022`/`20261023`): `Mom.affinities` are the `subject_kind='self'` concepts, `Mom.subject_affinities` the non-self ones, and `Mom.subject_concepts()` is the normalized `(subject_kind, concept_id)` set the matcher joins on; `BlendConfig` documents the serving/blend seat rule. |
| `stub_impl.py` | Reference implementation of all 4 ports. Pure functions, no I/O. **Faithful parity mirror of the deployed §C onion algorithm** (MAX-not-sum circle bonus, confirmed+grounded-only candidates, candidate set = ≥1 overlap arm, blocked pairs excluded, concept arm gated for dormancy, clamp [1,50], **concepts pair only within one `subject_kind`** — a child's karate never matches an adult's); its `transition_zip` is already a recount, matching the as-built `recount_zip_unlock`. Its one known divergence is the score-tie sort key — see "Known divergences" below. |
...
- **Onion scoring parity** (rev-2 §C) — a hand-seeded fixture asserting the stub reproduces the deployed algorithm: **MAX-not-sum** circle bonus (a peer sharing both a place and a type scores 3, never 4), component columns as a **split of the MAX** (so a place+type peer is `(3, 0)`, not `(3, 1)`), only **confirmed+grounded** rows score (suggested/dismissed/ungrounded never do), **p_limit clamp [1,50]**, **p_min_score** filter, the **concept-arm dormancy toggle**, and **subject-aware concept pairing** (a same-slug/different-subject peer is not a row at all; a same-subject peer scores +1 and reports its subject; `shared_concept_subjects[i]` describes `shared_concept_labels[i]`). Ordering is asserted on the reproducible key only (see "Known divergences" #1). This is the stub↔built regression gate.
...
3. **Subject arrays: empty is ambiguous, and the pairing rule is not directly observable.**
   Since `20261022120000` the RPC returns `shared_concept_subjects` parallel to
   `shared_concept_labels`. `MatcherLive` maps it with `.get(...) or ()`, so an **empty array
   means either "no shared concepts" or "this server predates the migration"** — the two are
   distinguishable only by `shared_concept_count > 0` alongside an empty subjects array, which
   a diff runner must treat as UNSCORED, never as a stub bug. Nor can the response prove the
   *pairing* rule: a wrongly-scored cross-subject pair and a correctly-scored same-subject one
   are the same row shape, so proving it live needs seeded claims of known `subject_kind` on
   both sides (`live_seed.py` writes no subject columns yet — that arm is stub-only for now).
   Two related non-divergences, for the record: `subject_name`/`subject_birth_year` are absent
   by **design** (owner-only, `20261021120000`'s privacy contract), and the label/subject
   arrays are ordered by `label` alone, so a label appearing under two subjects has an
   undefined server-side tie order — compare them as a **set of `(label, subject)` pairs**.
4. **`ring` (R1-R5) is a harness artifact.** The built matcher emits no ring; the stub derives
