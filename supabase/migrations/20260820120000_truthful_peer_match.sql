-- Truthful peer-match affinities (QA 2026-07-16: cards showed traits not on either
-- profile, plus inflated star counts).
--
-- 1. match_peers_by_claim_vectors(+_for_user): return the CALLER-side claim of the
--    best pair (matching_my_label) so surfaces can show the actual overlap instead of
--    presenting the peer's own claim as a shared trait. has_exact_concept_match now
--    means "the displayed pair shares the same concept slug" — it was previously true
--    when ANY claim concept matched anywhere, which let an unrelated shared boilerplate
--    claim (e.g. long_married on every QA account) mark every pairing as exact.
--    Exact-concept pairs are preferred for display; default similarity floor raised
--    0.65 → 0.70 (loose cosine pairs like "Enjoys Swimming"→"Enjoys sports" at ~0.76
--    read as invented affinities).
-- 2. find_my_fellows: stars/band come from the score alone (the exact-match boost to
--    5★/"PERFECT FIT" is gone), default floor raised 0.55 → 0.70, and the card label
--    is composed truthfully: same concept → "You both: <label>", otherwise
--    "You: <mine> · Them: <theirs>". trait_tags carry those claim labels.
--    Mirrors app/peer_discovery_surface.py — keep thresholds in sync.

-- Return signature changes (new out column) — drop first, callers are recreated below
-- or resolve at runtime (find_my_fellows, get_joint_moment_candidate select by name).
...
comment on function public.match_peers_by_claim_vectors(int, real) is
  ...
  'matching_my_label is the caller-side claim of that pair. Heritage/faith pairs '
  'require the same concept slug.';
...
-- Fellows drawer RPC: same return shape, truthful labels + score-only stars.
...
begin
  ...
  select
    ...
    -- Truthful reason: shared concept → one label both profiles carry; otherwise
    -- show both sides rather than presenting the peer's claim as shared.
    case
...
comment on function public.find_my_fellows(int, real) is
  ...
  '(stars/band/badge/trait_tags from the similarity score alone). Labels state the matched '
  'claim pair truthfully ("You both: X" / "You: X · Them: Y"). Identity fields masked '
  '(preview=true) until the caller is a verified non-anonymous neighbor. Raises '
  'home_block_missing when no home block is set.';
