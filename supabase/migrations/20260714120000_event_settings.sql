-- Host join-settings: capacity (max_attendees, already present), require-approval
-- (auto_approve, already present), and a new allow_attendee_share flag. create_event
-- now persists all three (it previously dropped auto_approve + had no share column).

alter table public.events
  add column if not exists allow_attendee_share boolean not null default true;
