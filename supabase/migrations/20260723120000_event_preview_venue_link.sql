-- Expose exact-place fields on the event preview so the shared /meet/[id] card
-- can turn the venue name into a tappable Google Maps link.
--
-- The events table already stores venue_address + place_id (20260713120000), but
-- the preview RPCs only returned venue_name. We add both to the returned JSONB.
-- place_id is the precise pin; venue_address is the human-readable fallback.
-- We intentionally do NOT expose lat/lng to keep exact coordinates off the
-- anonymous/non-attending preview.
