-- TagAlng: vector peer match on home block (pgvector cosine on claim embeddings).
-- Complements concept overlap in get_cluster_peers — finds similar meaning, not only same concept slug.

-- Authenticated: match peers for the signed-in user.
...
comment on function public.match_peers_by_claim_vectors(int, real) is
  'Top peers on caller home block by claim embedding similarity (public claims). p_min_similarity 0–1 (cosine).';

-- Service role: Lana worker / jobs (same logic, explicit user id).
...
comment on function public.match_peers_by_claim_vectors_for_user(uuid, int, real) is
  'Vector peer match for a given user (service role / Lana worker). Same block, public claims only.';
...
revoke all on function public.match_peers_by_claim_vectors_for_user(uuid, int, real) from public, anon, authenticated;
grant execute on function public.match_peers_by_claim_vectors_for_user(uuid, int, real) to service_role;
